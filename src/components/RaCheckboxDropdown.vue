...
function onChange(event: Event): void {
  options.value = options.value.map((item) => ({
    ...item,
    checked: item.key === (event.target as any).value ? (event.target as any).checked : item.checked
  }))
...
<template>
  <button
    ...
    class="inline-flex justify-center items-center px-4 py-2 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 w-full"
    type="button"
  >
    {{ props.label }}
    ...
    >
      <li v-for="option in options" :key="option.key">
        <div class="flex items-center p-2 rounded hover:bg-gray-100">
          ...
          />
          <label :for="option.key" class="w-full ms-2 text-sm font-medium text-gray-900 rounded">{{
            option.value
          }}</label>
        </div>
      </li>
